...
void main()
{
    ...
    vec3 diffuse = diffIntensity * vec3(1.0, 1.0, 1.0); 

    // Specular lighting
    vec3 viewDir = normalize(-fragPos); 
    vec3 reflectDir = reflect(-lightdir, normal);
    float specIntensity = pow(max(dot(viewDir, reflectDir), 0.0), 32.0);
    vec3 specular = specIntensity * vec3(1.0, 1.0, 1.0); 

    vec3 result = (ambient + diffuse + specular) * vec3(texture2D(tex, vTexCoord));
    if (isLightSource) {
